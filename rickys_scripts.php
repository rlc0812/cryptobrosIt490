...
{   $resultsFound = false;
    $conn = connectToDb();
    $username = mysqli_real_escape_string($array['username']);
    $password = mysqli_real_escape_string ($array['password']);
	...
	if((sizeof($all[0])) == 3){	//True if a set of data is found
	...
	if($resultsFound){//Will not execute anything if the query returns nothing
		...
		return $databaseUserID;//Gives ID to the front end for session validation
	    }

	    else {
		echo(" however password is not valid");
 		return false;
	    }
        }
	 else {
